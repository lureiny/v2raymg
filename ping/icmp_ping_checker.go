...
func (pi *pingerInfo) pingLoop(ch chan<- *PingResult) {
	...
	pi.pinger.OnFinish = func(s *ping.Statistics) {
		logger.Debug("OnFinish stop ping %s_%s_%s with ip[%v]",
			pi.nodeInfo.Geo, pi.nodeInfo.ISP, pi.nodeInfo.Host, pi.pinger.IPAddr().IP)
	}
	...
	for {
		select {
		case <-pi.ctx.Done():
			logger.Debug("recieve exit signal, stop ping[%s_%s_%s]",
				pi.nodeInfo.Geo, pi.nodeInfo.ISP, pi.nodeInfo.Host)
			pi.pinger.Stop()
			return
		case i := <-sendCh:
			pingPacket[i.Seq] = i
				...
				for _, node := range nodes {
					if pinger, ok := oldPingerMap[node.Host]; ok {
						...
						if err := pi.start(ipc); err != nil {
							logger.Error("start ping[%s_%s_%s] fail > %v", node.Geo, node.ISP, node.Host)
							continue
						}
