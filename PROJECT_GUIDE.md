...
- **2 P0 + 8 P1 簇中的 7 个 —— 已修复(2026-07-12,分支 `fix/p0-concurrency`,共 12 commit,70 文件 +3629/-321)**。每簇走 scout→design→红队对抗验证 + "修前必炸/修后必绿" 的 -race 回归;CI 已加 targeted `-race`。commit:P0-1 NodeManager `38a6620`、P0-2 hysteria `f4445c9`、A1 集群节点并发 `7a5ffbe`、A2 容器状态机+snell `13e5c25`、B2 SSRF/模板注入 `e0e0170`、B3 xray security=none `c425504`、C 转发数据面 `a2c3c08`、D 证书原子性 `571a7dd`、E 集群同步一致性 `ddffbe7`、F 进程收尸/二进制交换 `77106f6`、G 启动静默失败 `3851c81`、H 探测采集 `a93f985`。
  - **唯一未落地: P1-B1 集群 RPC 加密/鉴权**(encrypt_codec 无 KDF、crypto 无 AAD/重放、CenterNodeServer 明文无鉴权)。属**集群线格式破坏性变更**,混合版本滚动升级无回退路径,需用户定部署方式(A 协调升级 / B 先加版本协商 / C 仅加 token 长度校验)。决策文档:`docs/review-2026-07-10/B1-rpc-crypto-DECISION-NEEDED.md`。
