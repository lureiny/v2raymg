...
// 获取proto Node列表
func (cluster *Cluster) GetNodes(filterInvalid bool) map[string]*proto.Node {
	nodeMap := map[string]*proto.Node{}
	for key, node := range cluster.RemoteNode.GetNodes() {
		if filterInvalid && node.IsValid() {
...
func (cluster *Cluster) AuthRemoteNode(node **Node) error {
	...
	if n, ok := cluster.RemoteNode.HaveNode((*node).Name); !ok {
...
func (cluster *Cluster) Clear() {
	cluster.RemoteNode.Clear()
}

// 判断该节点是否有效
func (cluster *Cluster) IsValid(nodeName string) bool {
	return cluster.RemoteNode.Get(nodeName).IsValid()
}

// 判断节点node是否在本地注册过
func (cluster *Cluster) RegisteredLocal(nodeName string) bool {
	return cluster.RemoteNode.Get(nodeName).RegisteredLocal()
}

// 过滤超时未上报的节点
func (cluster *Cluster) Filter() {
	currentTime := time.Now().Unix()
	cluster.RemoteNode.Filter(getNodeFilterByCurrentTime(currentTime))
}

// 判断本地是否已经在远端节点注册
func (cluster *Cluster) RegisteredRemote(nodeName string) bool {
	return cluster.RemoteNode.Get(nodeName).RegisteredRemote()
}
