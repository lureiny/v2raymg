...
func TestFastAddInbound_Success_Shadowsocks(t *testing.T) {
	...
	err := exec.FastAddInbound("test-ss", map[string]any{
		"protocol": "shadowsocks",
		"method":   "2022-blake3-aes-256-gcm",
		// password omitted — auto-generated by FastAddInbound
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestFastAddInbound_SecurityDefaultForSS(t *testing.T) {
	...
	err := exec.FastAddInbound("secdef-ss-default", map[string]any{
		"protocol": "shadowsocks",
		"method":   "2022-blake3-aes-256-gcm",
		// password omitted — auto-generated
	})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	in, err := exec.GetInboundConfig("secdef-ss-default")
	if err != nil {
		t.Fatalf("failed to get inbound: %v", err)
	}
...
func TestFastAddInbound_TLS_CustomCert(t *testing.T) {
	...
	// Generate temp cert/key files (xray needs real PEM files to load the inbound)
	certFile, keyFile := generateTempSelfSignedCert(t)
...
func TestFastAddInbound_TLS_DomainCertFound(t *testing.T) {
	// Generate a temp self-signed cert/key for testing.
	certFile, keyFile := generateTempSelfSignedCert(t)

	exec := newIntegrationExecutorWith(t, func(cfg *ExecutorConfig) {
		cfg.CertManager = &mockCertManager{
			record: &certmgmtdomain.CertificateRecord{
				Domain:   "example.com",
				CertFile: certFile,
				KeyFile:  keyFile,
	...
	// Verify cert paths were injected from CertManager
	if xrayIn.extra["cert_file"] != certFile {
		t.Errorf("cert_file = %v, want %v", xrayIn.extra["cert_file"], certFile)
	}
	if xrayIn.extra["key_file"] != keyFile {
		t.Errorf("key_file = %v, want %v", xrayIn.extra["key_file"], keyFile)
	}
}
