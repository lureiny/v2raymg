...
// Relay listens on a local port and forwards TCP connections to a target address.
...
// Implementations wrap Read/Write to enforce throughput limits.
type Limiter interface {
	// Limit returns an io.Reader that reads from r at the limited rate.
	LimitReader(r io.Reader) io.Reader
	// Limit returns an io.Writer that writes to w at the limited rate.
...
// NewRelay creates a new Relay but does not start listening.
...
func (r *Relay) handleConn(clientConn net.Conn, remoteIP string) {
	defer r.wg.Done()
...
func (r *Relay) copyWithCount(dst io.Writer, src io.Reader, isUpload bool, onActivity func()) {
