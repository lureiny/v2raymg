...
// ─── DELETE /inbound container field validation ──────────────────────────────

func TestInboundDeleteHandler_ContainerValidation(t *testing.T) {
	cases := []struct {
		name       string
		body       map[string]interface{}
		wantStatus int
		...
		{
			name:       "no container defaults to xray — passes",
			body:       map[string]interface{}{"src_tag": "my-inbound"},
			wantStatus: 200,
		},
		{
			name:       "explicit xray — passes",
			body:       map[string]interface{}{"container": "xray", "src_tag": "my-inbound"},
			wantStatus: 200,
		},
		{
			name:       "snell container — rejected",
			body:       map[string]interface{}{"container": "snell", "src_tag": "my-inbound"},
			wantStatus: 400,
			wantBody:   "not supported",
		},
		{
			name:       "unknown container — rejected",
			body:       map[string]interface{}{"container": "wireguard", "src_tag": "my-inbound"},
			wantStatus: 400,
			wantBody:   "not supported",
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			router := newTestRouter(http.MethodDelete, "/inbound", func(c *gin.Context) {
				var req struct {
					Target    string `json:"target"`
				...
				if req.Container != "" && req.Container != "xray" {
					c.String(http.StatusBadRequest,
						"container %q not supported via DELETE /inbound; use POST /inbound/fast instead",
						req.Container)
					return
				}
				c.String(http.StatusOK, "validation_passed")
			...
			req := httptest.NewRequest(http.MethodDelete, "/inbound", bytes.NewReader(bodyBytes))
			req.Header.Set("Content-Type", "application/json")
			w := httptest.NewRecorder()
			router.ServeHTTP(w, req)
