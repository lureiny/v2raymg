...
func (s *CertificatesStorage) SaveResource(certRes *certificate.Resource) {
	...
	if err != nil {
		logger.Error("Unable to save Certificate for domain %s\n\t%v", domain, err)
	}

	if certRes.IssuerCertificate != nil {
		err = s.WriteFile(domain, ".issuer.crt", certRes.IssuerCertificate)
		if err != nil {
			logger.Error("Unable to save IssuerCertificate for domain %s\n\t%v", domain, err)
		}
	}
	...
	if certRes.PrivateKey != nil {
		...
		if err != nil {
			logger.Error("Unable to save PrivateKey for domain %s\n\t%v", domain, err)
		}
	} else if s.pem || s.pfx {
		// we don't have the private key; can't write the .pem or .pfx file
		logger.Debug("Unable to save PEM or PFX without private key for domain %s. Are you using a CSR?", domain)
	}

	jsonBytes, err := json.MarshalIndent(certRes, "", "\t")
	if err != nil {
		logger.Error("Unable to marshal CertResource for domain %s\n\t%v", domain, err)
	}

	err = s.WriteFile(domain, ".json", jsonBytes)
	if err != nil {
		logger.Error("Unable to save CertResource for domain %s\n\t%v", domain, err)
	}
}
...
func (s *CertificatesStorage) ReadResource(domain string) certificate.Resource {
	...
	if err != nil {
		logger.Error("Error while loading the meta data for domain %s\n\t%v", domain, err)
	}

	var resource certificate.Resource
	if err = json.Unmarshal(raw, &resource); err != nil {
		logger.Error("Error while marshaling the meta data for domain %s\n\t%v", domain, err)
	}
